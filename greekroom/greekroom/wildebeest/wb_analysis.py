...
def add_missing_default_argparse_args(args: argparse.Namespace) -> None:
    default_values = (('max_pattern_lines', 0),
...
def main():
    ...
    # legacy calls
    if args.json and isinstance(args.json, str) and args.json.endswith('scorecard/wildebeest.json'):
        args.json_legacy_out = io.StringIO(args.json)
        args.json_legacy_out.name = args.json
        args.json = None
    if (args.json_out_filename and isinstance(args.json_out_filename, str)
            and not args.json_out_filename.endswith('.json')):
